            ...
            <div class="flex flex-col bg-white rounded-2xl shadow-lg overflow-hidden border border-[#C4907C]/10 transition-transform hover:scale-[1.02]">
                <div class="relative h-56 w-full bg-[#EEF1EB]">
                    ...
                    <div class="absolute top-3 right-3 px-3 py-1 rounded-full text-[10px] uppercase font-black tracking-wider shadow-md 
                        {{ $car->status === 'available' ? 'bg-[#8BA89A]' : 'bg-red-400' }} text-white">
                        {{ $car->status === 'available' ? 'В наличии' : 'Продано' }}
                    </div>
                </div>
                <div class="p-6 flex flex-col flex-grow">
                    <div class="mb-4">
                        <h3 class="text-xl font-bold text-[#3D4047] leading-tight">{{ $car->brand }} {{ $car->model }}</h3>
                        <p class="text-[#7A7D82] text-sm font-medium">{{ $car->year }} года выпуска</p>
                    </div>
                    <p class="text-[#7A7D82] text-sm line-clamp-3 mb-6">{{ $car->description ?? 'Описание уточняйте у менеджера' }}</p>
                    <div class="mt-auto">
                        <div class="flex items-end justify-between mb-4">
                            <span class="text-[#7A7D82] text-xs uppercase font-bold">Цена выкупа</span>
                            <span class="text-2xl font-black text-[#C4907C]">{{ number_format($car->price, 0, '.', ',') }} ₽</span>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <article class="flex flex-col bg-[#3D4F5C] rounded-2xl overflow-hidden border border-white/5 transition-all hover:border-[#C4907C]/50 group">
                    <div class="relative h-52 overflow-hidden">
                        ...
                        <span class="absolute bottom-4 left-6 bg-[#8BA89A] text-white text-[10px] font-black uppercase px-3 py-1 rounded-full">{{ $post->category }}</span>
                    </div>
                    <div class="p-8 flex flex-col flex-grow">
                        <h3 class="text-xl font-bold text-white leading-snug group-hover:text-[#C4907C] transition-colors">{{ $post->title }}</h3>
                        <p class="text-gray-300 mt-4 text-sm leading-relaxed line-clamp-3">{{ strip_tags($post->content) }}</p>
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="bg-white p-10 rounded-3xl border border-[#C4907C]/10 relative flex flex-col shadow-2xl">
                    ...
                    <p class="text-[#3D4047]/70 italic leading-relaxed text-lg mb-8">"{{ $review->text }}"</p>
                    <div class="mt-auto border-t border-[#C4907C]/10 pt-6">
                        <div class="flex items-center justify-between">
                            <div>
                                <div class="font-bold text-[#3D4047] text-lg">{{ $review->client_name }}</div>
                                <div class="text-xs text-[#7A7D82] uppercase tracking-widest mt-1">{{ $review->car_model }}</div>
                            </div>
                            <div class="text-right">
                                <div class="text-[10px] text-[#7A7D82] uppercase font-bold">Выгода</div>
                                <div class="text-[#8BA89A] font-black text-xl">+{{ number_format($review->profit_amount, 0, '.', ' ') }} ₽</div>
                            </div>
                        </div>
                    </div>
                </div>
